...
$parsed = [System.Management.Automation.PsParser]::Tokenize(
    $content, [ref] $null) | Sort-Object StartLine, StartColumn
...
foreach ($token in $parsed) {
    ...
    if (($token.Type -eq "NewLine") -or ($token.Type -eq "LineContinuation")) {
        Write-CompleteLine $column
        Write-FormattedLine "{0:D3} {1} " ($token.StartLine + 1)
        $column = 1
    }
    else {
        ## Do any indenting
        ...
        if ( (($token.Type -eq "String") -or ($token.Type -eq "Comment")) -and ($token.EndLine -gt $token.StartLine)) {
            ...
            foreach ($stringLine in $stringLines) {
                ...
                if ($lineCounter -gt $token.StartLine) {
                    Write-CompleteLine $column
                    Write-FormattedLine "{0:D3} {1} " $lineCounter
                    $column = 1
                }
